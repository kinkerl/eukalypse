...
class Eukalypse:
	...
	def connect(self):
		return webdriver.Remote("%s/wd/hub"%self.host, desired_capabilities={"browserName": self.browser,"platform": self.platform,})
	...
	def compare(self, identifier, target_url, reference_image):
		...
		#if an ignoremask exist, multiply it with the difference. this makes everything black in the diff which is black in the ignoremask
		if self.ignoremask:
		...
		if notblack == 0:
			...
			diff_filename =os.path.join(self.output, "%s-difference.jpg" % identifier)
			...
			diff_imp_filename =os.path.join(self.output, "%s-difference-improved.jpg" % identifier)
